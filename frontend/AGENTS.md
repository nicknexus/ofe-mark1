...
- **Program page** (`src/pages/InitiativePage.tsx`) is a single surface: one header with a segmented control (Metrics, Logs, Locations, People, Stories) and three actions (**Add log**, **Quick setup**, **Report**). There is no per-program sidebar; the org `AppSidebar` stays on every authenticated page. Tab bodies use `h-full` (not `h-screen`) so they fit the page's flex column.
- **Structure lives in one place:** `src/components/setup/ProgramSetupDrawer.tsx` manages metrics, tags, locations and beneficiary groups for a program. Open it from the dashboard card ("Quick setup"), the program header, or onboarding. Do not add new one-off "add metric / add location" entry points; launch the drawer or the wizard's inline create instead.
- **Creating programs:** `CreateInitiativeModal` takes a `CreateInitiativeSource` (`blank | duplicate`) and `src/components/setup/createProgram.ts` turns that into the right API call.
