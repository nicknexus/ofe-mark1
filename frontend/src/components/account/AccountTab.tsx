...
export function AccountTab({
 ...
 {managingSubscription ? <><Spinner className="w-4 h-4" />Loading...</> : <><Settings className="w-4 h-4" />Manage<ExternalLink className="w-3 h-3" /></>}
 </button>
 )}
 </div>
 ...
 subscriptionStatus.subscription.status === 'active' ? 'bg-impact-100 text-impact-700' : 'bg-gray-100 text-gray-700'
 }`}>
 ...
 <button onClick={handleUpgrade} disabled={upgrading} className="app-btn app-btn-primary app-btn-sm">
 ...
 <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="app-input" placeholder="Your name" />
 </div>
 ...
 <div className={`rounded-xl shadow-card p-6 ${ownedOrganization.is_public ? 'bg-white border border-gray-100' : 'bg-amber-50 border-2 border-amber-200'}`}>
 <div className="flex items-center gap-3 mb-5">
 <div className={`relative p-2 rounded-xl ${ownedOrganization.is_public ? 'bg-impact-50' : 'bg-amber-100'}`}>
 {ownedOrganization.is_public ? <Globe className="w-5 h-5 text-impact-600" /> : <Lock className="w-5 h-5 text-amber-600" />}
 ...
 <div className="flex items-center gap-3">
 <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-sm font-medium ${ownedOrganization.is_public ? 'bg-impact-100 text-impact-700' : 'bg-gray-200 text-gray-700'}`}>
 ...
 className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${ownedOrganization.is_public ? 'bg-impact-500' : 'bg-gray-300'}`}
 >
 {updatingPublic ? (
 ...
 <div className="bg-gradient-to-r from-primary-50 to-primary-100/80 rounded-xl shadow-card border border-primary-100 p-6">
 ...
 <button onClick={() => setShowCreateOrg(true)} className="flex items-center gap-2 app-btn app-btn-primary app-btn-sm">
 ...
 <input type="text" value={newOrgName} onChange={(e) => setNewOrgName(e.target.value)} className="app-input" placeholder="Enter your organization name" required autoFocus />
 </div>
 <div className="flex items-center gap-3">
<button type="submit" disabled={creatingOrg} className="app-btn app-btn-primary flex items-center gap-2">
{creatingOrg ? <><Spinner className="w-4 h-4 border-white border-t-white/30" />Creating...</> : <><Rocket className="w-4 h-4" />Create & Start Trial</>}
 </button>
