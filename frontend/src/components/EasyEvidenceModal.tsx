...
export default function EasyEvidenceModal({
 ...
 notify.error('Please upload at least one file')
 return
 }

 const invalidFiles = selectedFiles.filter(f => !f.title.trim())
 if (invalidFiles.length > 0) {
 notify.error('Please enter a title for all files')
 return
 }
 ...
 notify.success(`Successfully added ${totalFiles} evidence ${totalFiles === 1 ? 'item' : 'items'}!`)
 if (capturedOnBatchComplete) {
 await capturedOnBatchComplete()
 }
 }
 } catch (err) {
 notify.error(`Failed to create evidence for ${fileWithType.file.name}`)
 }
 },
 onError: (error) => {
 if (error.message !== 'Upload cancelled') {
 notify.error(`Upload failed for ${fileWithType.file.name}`)
 }
 }
 })
 })
 ...
 panelClassName="bg-white md:border md:border-gray-200 md:rounded-xl w-full h-full md:w-full md:max-w-5xl md:h-auto md:min-h-[75vh] md:max-h-[95vh] shadow-app-modal overflow-hidden flex flex-col transition-all duration-300"
 animate={false}
 >
 {/* Header */}
 ...
 <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-white/25 flex items-center justify-center border border-white/30 flex-shrink-0">
 ...
 <div className="flex items-center gap-1.5 px-3 py-1.5 bg-primary-500 rounded-lg shadow-sm text-secondary-900">
 <Check className="w-3.5 h-3.5" />
 <span className="text-xs font-semibold">Auto-linked</span>
 </div>
 </div>
 {isRangeClaim && (
 ...
 {/* Progress Steps */}
 <div className="px-5 py-5 border-b border-gray-100 bg-gray-50 flex-shrink-0">
 ...
 ? 'bg-evidence-500 border-evidence-500 text-secondary-900 shadow-sm'
 : currentStep === step.number
 ? 'bg-evidence-500 border-evidence-500 text-secondary-900 ring-2 ring-evidence-200/50 shadow-sm'
 ...
 <div
 key={index}
 className="flex gap-5 p-4 app-card"
 >
 {/* Left: Preview */}
 ...
 className="app-btn bg-evidence-500 hover:bg-evidence-600 text-secondary-900 border-transparent flex items-center gap-2"
 >
 Next
