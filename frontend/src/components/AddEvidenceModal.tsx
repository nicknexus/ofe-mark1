...
export default function AddEvidenceModal({
 ...
 panelClassName="bg-white w-full h-full md:rounded-xl md:w-[80vw] md:max-w-[1300px] md:max-h-[97vh] md:h-[97vh] overflow-hidden shadow-app-modal md:border md:border-gray-200 transform transition-all duration-200 ease-out flex flex-col"
 >
 {/* Header */}
 <div className="flex items-center justify-between px-4 md:px-6 py-2.5 md:py-3 border-b border-evidence-200/40 bg-gradient-to-r from-evidence-50 to-evidence-50/80 flex-shrink-0">
 <div className="flex items-center space-x-3 flex-1 min-w-0">
 <div className="w-8 h-8 rounded-lg bg-evidence-500/15 flex items-center justify-center border border-evidence-300/30 flex-shrink-0">
 ...
 <button
 onClick={onClose}
 className="text-gray-400 hover:text-gray-600 p-1.5 rounded-lg hover:bg-gray-100 transition-all duration-200 ml-2 md:ml-4 flex-shrink-0"
 >
 <X className="w-5 h-5" />
 </button>
 </div>
 ...
 <div className="px-4 md:px-6 py-2 md:py-3.5 border-b border-evidence-100/40 bg-gray-50 flex-shrink-0">
 ...
 <div className={`flex items-center justify-center w-9 h-9 rounded-xl border-2 transition-all duration-200 ${currentStep > step.number
 ...
 <div className={`text-sm font-medium whitespace-nowrap ${currentStep >= step.number ? 'text-gray-700' : 'text-gray-400'
 }`}>
 {step.title}
 </div>
 </div>
 {index < steps.length - 1 && (
 <div className={`h-0.5 w-10 lg:w-16 mx-3 rounded-full transition-all duration-200 ${currentStep > step.number ? 'bg-evidence-500' : 'bg-gray-200/60'
 }`} />
 )}
 </React.Fragment>
 ))}
 ...
 <label
 key={value}
 className={`relative flex items-center gap-2 p-2.5 border-2 rounded-lg cursor-pointer transition-all duration-150 ${formData.type === value
 ...
 <button
 type="button"
 onClick={() => setShowCoverageHelp(s => !s)}
 className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-700 transition-colors"
 >
 <span className="w-4 h-4 rounded-full border border-gray-400 flex items-center justify-center font-bold text-gray-500 flex-shrink-0">?</span>
 How does evidence matching &amp; coverage work?
 </button>
 {showCoverageHelp && (
 <div className="mt-2 rounded-xl bg-primary-50 border border-primary-200 p-3 text-xs text-primary-900 leading-relaxed">
 Evidence is automatically linked to impact claims when their <strong>date and location overlap</strong>. Select the date and location below that match the claim you want to support. Each matched claim increases your metric's <strong>coverage score</strong> — the percentage of claims backed by at least one piece of evidence.
 </div>
 )}
 </div>
 ...
 <label
 key={location.id}
 className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border-2 transition-colors cursor-pointer ${isChecked
 ...
 <label
 key={kpi.id}
 className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border-2 transition-colors ${isDisabled
 ...
 {isPreSelected && (
 <span className="inline-flex items-center px-1.5 py-0.5 app-icon-tile app-icon-tile-accent text-xs font-medium rounded-full flex-shrink-0">
 Pre-selected
 </span>
 )}
 {'total_updates' in kpi && kpi.total_updates > 0 && (
 <span className="inline-flex items-center justify-center w-4 h-4 bg-primary-500 text-secondary-900 text-xs font-bold rounded-full flex-shrink-0">
 ...
 <button
 type="button"
 className="text-xs px-2 py-1 bg-white hover:bg-evidence-50 border border-evidence-200 rounded-md text-evidence-700 font-medium transition-colors"
 onClick={() => {
 ...
 <label
 key={group.id}
 className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border-2 transition-colors cursor-pointer ${checked
 ? 'bg-primary-50 border-primary-300'
 : 'bg-white border-gray-200 hover:border-primary-300 hover:bg-gray-50'
 }`}
 >
 ...
 <div className="bg-evidence-50 border-2 border-evidence-200 rounded-xl p-4 mb-4">
 <p className="text-xs text-evidence-700 leading-relaxed mb-2">
 <strong>Evidence Type:</strong> {evidenceTypes.find(et => et.value === formData.type)?.label}
 </p>
 <p className="text-xs text-evidence-700 leading-relaxed">
 ...
 <div className="space-y-3">
 <File className="w-12 h-12 text-primary-700 mx-auto" />
 <p className="text-base font-semibold text-evidence-700">Current file attached</p>
 ...
 {/* Navigation Footer */}
 <div className="border-t border-evidence-100/40 px-4 md:px-6 py-2.5 bg-gray-50 flex-shrink-0">
 ...
 {uploadProgress && (
 <div className="px-3 py-1 text-xs text-evidence-700 bg-evidence-50 rounded-lg border border-evidence-200/60">
 {uploadProgress}
 </div>
 )}
 ...
 className="app-btn app-btn-sm bg-evidence-500 hover:bg-evidence-600 text-secondary-900 border-transparent relative z-10"
 >
 <span>Next</span>
