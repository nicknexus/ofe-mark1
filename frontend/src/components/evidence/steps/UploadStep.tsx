...
export default function UploadStep({ files, onAddFiles, onRemoveFile, onAddLink }: UploadStepProps) {
 ...
 className={`border-2 border-dashed rounded-xl text-center cursor-pointer transition-all flex-shrink-0 ${
 ...
 >
 <div className={`mx-auto rounded-xl bg-evidence-500/10 border border-evidence-300/40 flex items-center justify-center ${
 ...
 <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${f.isLink ? 'bg-primary-100' : 'bg-evidence-100'}`}>
 {f.isLink
 ? <LinkIcon className="w-4 h-4 text-primary-700" />
 ...
 {f.status === 'done' && <Check className="w-4 h-4 text-impact-600 flex-shrink-0" />}
