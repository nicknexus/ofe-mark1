...
export default function MobileMetricsTab({ initiativeId, autoAdd }: MobileMetricsTabProps) {
 ...
 description="Create metrics on desktop to start tracking impact."
 />
 ) : (
 ...
 className="app-card-interactive w-full p-4 text-left active:scale-[0.98]"
 >
 ...
 kpi.category === 'output' ? 'bg-evidence-50 text-primary-700' :
...
function MobileMetricDetail({ kpi, initiativeId, onBack, autoAdd }: MetricDetailProps) {
 ...
 <EmptyState
 className="app-card py-10"
 icon={BarChart3}
 title="No Impact Claims"
 description="Add your first data point for this metric."
 action={
 canAddImpactClaims ? (
 <button type="button" onClick={() => setShowAddForm(true)} className="app-btn app-btn-primary">
 Add Impact Claim
 </button>
 ) : undefined
 }
 ...
 .map((update) => (
 <div key={update.id} className="app-card p-3.5">
...
function MobileAddClaimFlow({ kpi, initiativeId, onClose, onSuccess }: AddClaimFlowProps) {
 ...
 notify.success('Impact claim added!')
 onSuccess()
 } catch (error) {
 ...
 autoFocus
 className="app-input w-full text-center text-4xl font-bold py-6 border-2"
 />
