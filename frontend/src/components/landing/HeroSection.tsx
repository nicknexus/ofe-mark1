...
const FloatingCard = ({
  ...
  return (
    ...
    >
      <div
        className={`absolute -top-6 -left-6 z-20 w-14 h-14 sm:w-16 sm:h-16 rounded-2xl overflow-hidden shadow-glass-lg ring-[3px] ring-white/90 bg-gradient-to-br ${gradient}`}
        style={{ backgroundColor: `${accent}26` }}
      >
        {claim.org_logo_url ? (
          <img src={claim.org_logo_url} alt="" className="w-full h-full object-cover" />
        ) : (
...
const HeroSection = ({ onGetStarted }: HeroSectionProps) => {
  ...
  const claimPool = claims.length > 0 ? claims : FALLBACK_CLAIMS;
