...
import type { ImpactClaimCategoryVisual } from './impactClaimCategoryConfig'

export function PublicImpactClaimEvidenceGallerySection({ evidence, evidenceCount, config, galleryIndex, setGalleryIndex, currentFileIndex, setCurrentFileIndex, orgSlug, initiativeSlug, tagsById }: {
    evidence: PublicEvidence[]
    evidenceCount: number
    config: ImpactClaimCategoryVisual
    ...
    const typeConfig: Record<string, { bg: string; label: string; color: string; icon: any }> = {
        ...
        documentation: { bg: 'bg-evidence-100 text-evidence-700', label: 'Documentation', color: 'text-evidence-500', icon: FileText },
                        ...
                        <div className="flex items-center justify-between mb-3 sm:mb-4 flex-shrink-0 gap-2">
                            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                                <button onClick={closeGallery} className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors flex-shrink-0">
                                    ...
                                    <span className="text-sm font-medium hidden sm:inline">Back to Evidence</span>
                                </button>
                                ...
                                <span className="text-muted-foreground text-sm hidden sm:inline flex-shrink-0">{galleryIndex + 1} of {filteredEvidence.length}</span>
                            </div>
