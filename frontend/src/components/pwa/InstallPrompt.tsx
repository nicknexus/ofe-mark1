...
export default function InstallPrompt({ onLogout }: InstallPromptProps) {
 ...
 <div className="w-20 h-20 rounded-xl overflow-hidden mx-auto mb-4 shadow-lg">
...
function IOSSafariInstructions() {
 ...
 <span className="inline-flex items-center ml-1.5 text-evidence-600">
 <ArrowUp className="w-5 h-5 p-0.5 border border-primary-500 rounded" />
 </span>
...
function IOSOtherInstructions({ onCopyUrl, copied }: { onCopyUrl: () => void; copied: boolean }) {
 ...
 >
 {copied ? <Check className="w-4 h-4 text-impact-600" /> : <Copy className="w-4 h-4" />}
...
function AndroidOtherInstructions() {
 return (
 <div className="space-y-4">
 <div className="bg-evidence-50 border border-evidence-200 rounded-xl p-4 text-sm text-evidence-700">
...
function Step({ number, children }: { number: number; children: React.ReactNode }) {
 return (
 <div className="flex items-start gap-3 app-card p-4">
