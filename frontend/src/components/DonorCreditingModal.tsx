...
export default function DonorCreditingModal({
 ...
 notify.error('Please fix all invalid credit amounts before saving')
 return
 }
 setLoading(true)
 try {
 ...
 if (newTotal > max) {
 notify.error(`Credited value for ${dashboard?.kpis.find(k => k.id === kpiId)?.title || 'metric'} exceeds available amount. Available: ${available.toFixed(2)}`)
 setLoading(false)
 return
 }
 ...
 notify.error(`Credited value for ${metric?.title || 'metric'} exceeds available amount`)
 setLoading(false)
 return
 }
 }
 ...
 className={`flex-1 px-2 py-1 text-sm border rounded focus:ring-1 focus:ring-primary-500 focus:border-transparent ${
