        ...
        <div className="space-y-6">
            ...
            {kpis.length === 0 ? (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    {/* Left Column - KPIs (Takes more space) */}
                    <div className="xl:col-span-2 space-y-6">
                        {/* Quick Stats Bar */}
                        <div className="grid grid-cols-3 gap-4">
                            <div className="card p-4 text-center">
                                <p className="text-2xl font-bold text-primary-600">{stats.total_kpis}</p>
                                <p className="text-sm text-gray-600">Active KPIs</p>
                            </div>
                            <div className="card p-4 text-center">
                                <p className="text-2xl font-bold text-green-600">{stats.evidence_coverage_percentage}%</p>
                                <p className="text-sm text-gray-600">Evidence Coverage</p>
                            </div>
                            <div className="card p-4 text-center">
                                <p className="text-2xl font-bold text-blue-600">{stats.recent_updates}</p>
                        ...
                        <div className="card p-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-semibold text-gray-900">Your KPIs</h3>
                                <button
                                    onClick={() => setIsKPIModalOpen(true)}
                                    className="btn-secondary flex items-center space-x-2"
                                >
                            ...
                            <div className="space-y-3">
                                {kpis.map((kpi) => (
                                    <div key={kpi.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-shadow">
                                        <div className="flex items-center justify-between mb-3">
                                            <div className="flex-1">
                                                <div className="flex items-center space-x-3 mb-1">
                                                    <h4 className="font-medium text-gray-900">{kpi.title}</h4>
                                                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getCategoryColor(kpi.category)}`}>
                                                        {kpi.category}
                                                    </span>
                                                </div>
                                                <p className="text-sm text-gray-600 line-clamp-1">{kpi.description}</p>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex space-x-2">
                                                <button
                                                    ...
                                                    className="px-3 py-1 bg-primary-50 text-primary-700 rounded text-sm hover:bg-primary-100 transition-colors"
                                                >
                                                    Add Data
                                                </button>
                                                {kpi.total_updates > 0 && (
                                                ...
                                                <Link
                                                    ...
                                                    className="px-3 py-1 text-gray-600 hover:text-gray-800 rounded text-sm hover:bg-gray-100 transition-colors"
                                                >
                                                    View Details
                                                </Link>
                                            </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="card p-4">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center space-x-3">
                                    ...
                                    <span className="text-sm text-gray-700">{kpis.length} KPIs created</span>
                                </div>
                                <div className="flex items-center space-x-3">
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${stats.recent_updates > 0 ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-600'
                                        }`}>
                                ...
                                <div className="flex items-center space-x-3">
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${stats.evidence_coverage_percentage > 0 ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-600'
                                        }`}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="card p-4">
                                <h4 className="font-medium text-gray-900 mb-3">Evidence Coverage</h4>
                                <div className="space-y-2">
                                    {kpis.slice(0, 4).map((kpi) => (
                                        <div key={kpi.id} className="flex items-center justify-between">
                                            <span className="text-sm text-gray-600 line-clamp-1 flex-1 mr-2">
                                                {kpi.title.length > 20 ? kpi.title.substring(0, 20) + '...' : kpi.title}
                                            </span>
                                            <div className="flex items-center space-x-2">
                                                <div className="w-16 bg-gray-200 rounded-full h-2">
                                                    <div
                                                        className={`h-2 rounded-full ${kpi.evidence_percentage >= 80 ? 'bg-green-500' :
                                                                kpi.evidence_percentage >= 30 ? 'bg-yellow-500' : 'bg-red-500'
                                                            }`}
                                                        style={{ width: `${kpi.evidence_percentage}%` }}
                                                    ></div>
                                                </div>
                                                <span className="text-xs text-gray-500 w-8">{kpi.evidence_percentage}%</span>
                                            </div>
                                        </div>
                                    ))}
                                    {kpis.length > 4 && (
