...
export default function AdminDemosPage() {
 ...
 notify.success(`Created "${demo.name}" with baseline content`)
 setNewName('')
 setNewFolder('')
 setShowCreate(false)
 await refresh()
 ...
 if (!url) {
 notify.error('Website URL is required')
 return
 }
 ...
 // 2. Generate content into the shell in the background; the app-card shows a spinner.
 try {
 await AdminApi.generateInto(shellId, { website_url: url, name })
 notify.success(`Generated "${name}"`)
 } catch (err) {
 notify.error(`Generation failed: ${(err as Error).message}`)
 } finally {
 await refresh()
 }
 }
 ...
 if (!demoName) {
 notify.error('Enter a name for the first demo charity in this folder')
 return
 }
 setCreatingFolder(true)
 try {
 await AdminApi.createDemoShell({ name: demoName, demo_folder: folder })
 notify.success(`Created folder "${folder}" with "${demoName}"`)
 ...
 notify.success(`Generated "${demo.name}"`)
 } catch (err) {
 notify.error(`Generation failed: ${(err as Error).message}`)
 } finally {
 await refresh()
 ...
 notify.success(folder ? `Moved to "${folder}"` : 'Removed from folder')
 } catch (err) {
 notify.error((err as Error).message)
 }
 }
 ...
 notify.success(`Cloned to "${newDemo.name}"`)
 await refresh()
 } catch (err) {
 notify.error((err as Error).message)
 }
 ...
 notify.success(`Deleted "${demo.name}"`)
 await refresh()
 } catch (err) {
 notify.error((err as Error).message)
 }
 ...
 notify.success(next ? 'Public share link enabled' : 'Public share link disabled')
 setDemos((prev) => prev.map((d) => (d.id === demo.id ? { ...d, ...updated } : d)))
 } catch (err) {
 notify.error((err as Error).message)
 }
 ...
 <div>
 <h1 className="app-page-title">Demo Charities</h1>
 ...
 <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-evidence-50 text-evidence-700 rounded-full text-xs font-semibold">
 ...
 <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-impact-100 text-impact-700 rounded-full text-xs font-semibold uppercase tracking-wide">
 ...
 ? 'text-impact-700 hover:bg-impact-50'
 : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100'
 }`}
 >
 ...
 <ModalFrame
 zIndexClass="z-[90]"
 panelClassName="bg-white rounded-xl max-w-md w-full shadow-modal"
 >
 <div className="p-5">
 <div className="flex items-center justify-between mb-4">
...
function FolderField({
 ...
 <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-gray-100 z-50 py-1 max-h-64 overflow-y-auto">
