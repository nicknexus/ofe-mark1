...
const TEXT_FIELDS: TextFieldConfig[] = [
 ...
 icon: Lightbulb,
 accent: 'bg-impact-50 text-impact-600',
 },
 {
 key: 'additional_info',
 label: 'More Context',
 ...
 icon: FileText,
 accent: 'bg-evidence-50 text-evidence-600',
 },
]
...
export default function OrgContextPage() {
 ...
 notify.error('Every stat card needs a number/value. Fill it in or switch it to a statement.')
 return
 }
 }
 ...
 notify.error('Featured video must be a YouTube or Vimeo URL.')
 return
 }
 ...
 notify.error(`You've reached the max of ${MAX_STAT_CARDS} stats`)
 return
 }
 ...
 notify.error(`You've reached the max of ${MAX_THEORY_STAGES} stages`)
 return
 }
 ...
 notify.error(`You've reached the max of ${MAX_STRATEGIES} strategies`)
 return
 }
 ...
 <InlineAlert tone="warning" title="Read-only view" className="mb-6">
 Join an organization to edit these fields.
 </InlineAlert>
 )}

 {error && !showLoader && (
 <InlineAlert tone="error" className="mb-6">{error}</InlineAlert>
 )}
...
function StatCardEditor({
 ...
 maxLength={20}
 className="app-input text-2xl font-bold read-only:bg-gray-50 read-only:cursor-not-allowed"
 />
 </div>
 )}
 <div>
